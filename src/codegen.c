...
static int emit_fast_note(FILE *out);
...
static int emit_rt_dynamic_fast(FILE *out)
{
    if (!emit_fast_note(out)) return 0;
    ...
    if (!emit_str(out,
                ...
                "    int idx, t, i, dv;\n"
                "    int set_dir = 0, ndir = dir, do_skip = 0, do_cond = 0, do_tp = 0, tpx = 0, tpy = 0, tpd = dir;\n"
                "    if (!in_bounds(x, y)) VM_FAIL(\"ERR_OUT_OF_BOUNDS\");\n"
                "    idx = (y * WIDTH + x) * 4;\n"
                "    decode_pixel(img[idx + 0], img[idx + 1], img[idx + 2], img[idx + 3], &t, &i, &dv);\n"
                "    if (t == TYPE_HALT) return 0;\n"
                "    if (t == TYPE_DATA)\n"
                "    {\n"
                "      if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\");\n"
                "      st[sp++] = dv;\n"
                "    }\n"
                "    else if (t == TYPE_CODE)\n"
                "    {\n"
                ...
                "        case I_UP: set_dir = 1; ndir = 3; break;\n"
                "        case I_UP_SKIP: set_dir = 1; ndir = 3; do_skip = 1; break;\n"
                "        case I_POP: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); sp--; break;\n"
                "        case I_SWAP: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[sp - 1]; st[sp - 1] = st[sp - 2]; st[sp - 2] = u; break;\n"
                "        case I_ADD: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v + u; break;\n"
                "        case I_SUB: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v - u; break;\n"
                "        case I_MUL: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v * u; break;\n"
                "        case I_DIV: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; if (u == 0) VM_FAIL(\"ERR_DIV_ZERO\"); st[sp++] = v / u; break;\n"
                "        case I_MOD: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; if (u == 0) VM_FAIL(\"ERR_MOD_ZERO\"); st[sp++] = v % u; break;\n"
                "        case I_NEG: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); st[sp - 1] = -st[sp - 1]; break;\n"
                "        case I_INC: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); st[sp - 1]++; break;\n"
                "        case I_DEC: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); st[sp - 1]--; break;\n"
                "        case I_STORE_A: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); a = st[--sp]; break;\n"
                "        case I_LOAD_A: if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = a; break;\n"
                "        case I_STORE_B: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); b = st[--sp]; break;\n"
                "        case I_LOAD_B: if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = b; break;\n"
                "        case I_STORE_C: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); c = st[--sp]; break;\n"
                "        case I_LOAD_C: if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = c; break;\n"
                "        case I_STORE_D: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); d = st[--sp]; break;\n"
                "        case I_LOAD_D: if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = d; break;\n"
                "        case I_JNZ: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); do_cond = (st[--sp] != 0); break;\n"
                "        case I_JZ: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); do_cond = (st[--sp] == 0); break;\n"
                "        case I_JGT: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; do_cond = (v > u); break;\n"
                "        case I_JLT: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; do_cond = (v < u); break;\n"
                "        case I_CALL:\n"
                "          if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\");\n"
                "          if (csp >= CALL_MAX) VM_FAIL(\"ERR_CALL_OVERFLOW\");\n"
                "          u = st[--sp]; v = st[--sp]; rx = x; ry = y; move_once(&rx, &ry, dir);\n"
                "          cx[csp] = rx; cy[csp] = ry; cd[csp] = dir; csp++;\n"
                "          do_tp = 1; tpx = v; tpy = u; tpd = dir;\n"
                "          break;\n"
                "        case I_RET:\n"
                "          if (csp <= 0) VM_FAIL(\"ERR_CALL_UNDERFLOW\");\n"
                "          csp--; do_tp = 1; tpx = cx[csp]; tpy = cy[csp]; tpd = cd[csp];\n"
                "          break;\n"
                "        case I_DUP: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp] = st[sp - 1]; sp++; break;\n"
                "        case I_OVER: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = st[sp - 2]; break;\n"
                "        case I_ROT: if (sp < 3) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[sp - 3]; st[sp - 3] = st[sp - 2]; st[sp - 2] = st[sp - 1]; st[sp - 1] = u; break;\n"
                "        case I_ROTR: if (sp < 3) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[sp - 1]; st[sp - 1] = st[sp - 2]; st[sp - 2] = st[sp - 3]; st[sp - 3] = u; break;\n"
                "        case I_READ:\n"
                "          if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\");\n"
                "          u = st[--sp]; v = st[--sp];\n"
                "          if (!in_bounds(v, u)) VM_FAIL(\"ERR_MEM_OOB\");\n"
                "          mi = (u * WIDTH + v) * 4;\n"
                "          if (img[mi + 3] == 0) VM_FAIL(\"ERR_MEM_ALPHA\");\n"
                "          if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\");\n"
                "          st[sp++] = max3(img[mi], img[mi + 1], img[mi + 2]);\n"
                "          break;\n"
                "        case I_WRITE:\n"
                "          if (sp < 3) VM_FAIL(\"ERR_STACK_UNDERFLOW\");\n"
                "          u = st[--sp]; v = st[--sp]; mi = st[--sp];\n"
                "          if (!in_bounds(v, u)) VM_FAIL(\"ERR_MEM_OOB\");\n"
                "          if (mi < 0) mi = 0; if (mi > 255) mi = 255;\n"
                "          idx = (u * WIDTH + v) * 4; img[idx] = (unsigned char)mi; img[idx + 1] = (unsigned char)mi; img[idx + 2] = (unsigned char)mi; img[idx + 3] = 255;\n"
                "          break;\n"
                "        case I_AND: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v & u; break;\n"
                "        case I_OR: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v | u; break;\n"
                "        case I_NOT: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); st[sp - 1] = ~st[sp - 1]; break;\n"
                "        case I_XOR: if (sp < 2) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); u = st[--sp]; v = st[--sp]; st[sp++] = v ^ u; break;\n"
                "        case I_DEPTH: if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = sp; break;\n"
                "        case I_CLEAR: sp = 0; break;\n"
                "        case I_IN_NUM: if (scanf(\"%d\", &u) != 1) VM_FAIL(\"ERR_INPUT\"); if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = u; break;\n"
                "        case I_IN_CHR: u = getchar(); if (u == EOF) VM_FAIL(\"ERR_INPUT\"); if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\"); st[sp++] = (unsigned char)(u & 0xFF); break;\n"
                "        case I_OUT_NUM: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); printf(\"%d\", st[--sp]); fflush(stdout); break;\n"
                "        case I_OUT_CHR: if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\"); putchar((unsigned char)(st[--sp] & 0xFF)); fflush(stdout); break;\n"
                "        case I_DEBUG:\n"
                "          if (sp > 0) printf(\"DEBUG: SP=%d TOP=%d A=%d B=%d C=%d D=%d\\n\", sp, st[sp - 1], a, b, c, d);\n"
                "          else printf(\"DEBUG: SP=%d TOP=EMPTY A=%d B=%d C=%d D=%d\\n\", sp, a, b, c, d);\n"
                "          break;\n"
                "        case I_TRAP_00: VM_FAIL(\"ERR_TRAP_00\");\n"
                "        case I_TRAP_01: VM_FAIL(\"ERR_TRAP_01\");\n"
                "        case I_TRAP_02: VM_FAIL(\"ERR_TRAP_02\");\n"
                "        case I_TRAP_03: VM_FAIL(\"ERR_TRAP_03\");\n"
                "        case I_TRAP_04: VM_FAIL(\"ERR_TRAP_04\");\n"
                "        case I_TRAP_05: VM_FAIL(\"ERR_TRAP_05\");\n"
                "        case I_TRAP_06: VM_FAIL(\"ERR_TRAP_06\");\n"
                "        case I_TRAP_07: VM_FAIL(\"ERR_TRAP_07\");\n"
                "        case I_TRAP_08: VM_FAIL(\"ERR_TRAP_08\");\n"
                "        case I_TRAP_09: VM_FAIL(\"ERR_TRAP_09\");\n"
                "        case I_TRAP_10: VM_FAIL(\"ERR_TRAP_10\");\n"
                "        case I_TRAP_11: VM_FAIL(\"ERR_TRAP_11\");\n"
                "        case I_TRAP_12: VM_FAIL(\"ERR_TRAP_12\");\n"
                "        case I_TRAP_13: VM_FAIL(\"ERR_TRAP_13\");\n"
                "        case I_NOP: break;\n"
                "      }\n"
                "    }\n"
                "    if (do_tp)\n"
                ...
                "    move_once(&x, &y, dir);\n"
                "    if (!in_bounds(x, y)) VM_FAIL(\"ERR_OUT_OF_BOUNDS\");\n"
                "    if (do_skip) { move_once(&x, &y, dir); if (!in_bounds(x, y)) VM_FAIL(\"ERR_OUT_OF_BOUNDS\"); }\n"
                "    if (do_cond) { move_once(&x, &y, dir); if (!in_bounds(x, y)) VM_FAIL(\"ERR_OUT_OF_BOUNDS\"); }\n"
                "  }\n"
                "}\n"))
        return 0;
...
static int emit_fast_note(FILE *out)
{
    return emit_str(out,
                    "\n"
                    "/* Fast mode note:\n"
                    " * Core hard checks remain enabled (stack/div/mod/memory/trap).\n"
                    " * Verbose diagnostics are reduced for smaller generated code.\n"
                    " */\n"
                    "#ifdef FAST_C\n"
                    "#define VM_FAIL(msg) return 1\n"
                    "#else\n"
                    "#define VM_FAIL(msg) do { fprintf(stderr, \"EXEC_ERROR: %s\\n\", msg); return 1; } while (0)\n"
                    "#endif\n\n");
}

static int emit_rt_fast(FILE *out)
{
    if (!emit_fast_note(out)) return 0;
    ...
    if (fprintf(out,
                ...
                "    if (type == %d) return 0;\n"
                "\n"
                "    if (type == %d) VM_FAIL(\"ERR_TRANSPARENT_PIXEL\");\n"
                "\n"
                "    if (type == %d)\n"
                "    {\n"
                "      if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\");\n"
                ...
                "\n"
                "        case %d:\n"
                "          if (sp <= 0) VM_FAIL(\"ERR_STACK_UNDERFLOW\");\n"
                "          sp--;\n"
                "          break;\n"
                "\n"
                "        case %d:\n"
                ...
                "\n"
                "        case %d:\n"
                "          if (scanf(\"%%d\", &a) != 1) VM_FAIL(\"ERR_INPUT\");\n"
                "          if (sp >= STACK_MAX) VM_FAIL(\"ERR_STACK_OVERFLOW\");\n"
                "          stack[sp++] = a;\n"
                "          break;\n"
                "\n"
...
{
    int fast_c = (options && options->fast_c) ? 1 : 0;
    ...
    if (image_has_ex(img, width, height))
    {
        ...
        if (!emit_img_rgba(out_ex, img, width, height) ||
            !(fast_c ? emit_rt_dynamic_fast(out_ex) : emit_rt_dynamic(out_ex)))
        {
            fclose(out_ex);
            return 0;
        }
    ...
    if (!(fast_c ? emit_rt_fast(out) : emit_rt(out)))
    {
        fclose(out);
        ir_free(&ir);
        return 0;
